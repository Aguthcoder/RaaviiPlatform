...
export class AttendanceService {
  ...
  private async handleNoShow(user: User, eventId: string) {
    ...
    if (noShowCount === 1) {
      ...
      console.log(`⚠️ WARNING: User ${user.id} (${user.mobileNumber}) has 1 no-show for event ${eventId}`);
      // TODO: ارسال SMS هشدار
    } else if (noShowCount >= 2) {
      // بن اکانت
      await this.usersRepo.update(user.id, { isBanned: true } as any);
      console.log(`🚫 BANNED: User ${user.id} (${user.mobileNumber}) banned after 2 no-shows`);
      // TODO: ارسال SMS بن
    }
  }
  ...
  async triggerRatingRequest(eventId: string) {
    ...
    console.log(`📧 Rating request triggered for event ${eventId}: ${participants.length} participants`);
