...
export default function VerifyPage() {
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Check if profile incomplete → force profile completion
      const hasName = !!(user.name?.trim());
      const hasCity = !!(user.city || user.profile?.city);
      if (!hasName || !hasCity) {
      ...
      try {
        ...
        if (isAdminPhone(phone)) { router.replace("/admin/dashboard"); return; }
        router.replace("/dashboard/complete-profile");
      } catch {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative z-10">
      <div className="w-full max-w-md p-8 rounded-3xl shadow-2xl relative z-10"
