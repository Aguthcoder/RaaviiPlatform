...
export default function ProfilePage() {
  ...
  async function onSave(e: React.FormEvent) {
    ...
    setIsSaving(true);
    try {
      // فقط فیلدهایی که بک‌اند قبول می‌کند ارسال کن (null/undefined حذف شود)
      ...
      const parsedInterests = interestsInput
        ...
        .map((i) => i.trim())
        .filter(Boolean);
      payload.interests = parsedInterests;
  ...
  const displayAvatar =
    avatarPreview || profile.avatarUrl || state.user?.avatar || "";
  ...
  return (
    <div className="max-w-2xl mx-auto pb-28 space-y-5">
      {/* ── کارت هدر پروفایل ── */}
      <div
        className="rounded-3xl overflow-hidden shadow-2xl border border-slate-700/50"
        style={{
      ...
      >
        ...
        >
          <div
            ...
            style={{
              backgroundImage:
                "radial-gradient(ellipse at 20% 50%, rgba(255,255,255,0.15) 0%, transparent 60%), radial-gradient(ellipse at 80% 20%, rgba(255,200,100,0.1) 0%, transparent 50%)",
            }}
          />
          <div
      ...
      <div className="grid grid-cols-2 gap-3">
        {statCards.map(({ icon: Icon, value, label, color, bg }) => (
          ...
          >
            <div
              className={`w-11 h-11 rounded-xl bg-${color}-500/20 flex items-center justify-center`}
            >
      ...
      {isEditing && (
        ...
        >
          ...
          {/* آپلود عکس */}
          <div
            className="border-2 border-dashed border-slate-600 rounded-2xl p-5 text-center hover:border-orange-500/50 transition-colors cursor-pointer"
            onClick={() => fileInputRef.current?.click()}
          >
            <div className="flex flex-col items-center gap-2">
              ...
              <div className="flex items-center gap-2 text-orange-400 font-bold text-sm">
                <Upload size={15} />
                <span>
                  {uploadingAvatar ? "در حال آپلود..." : "انتخاب عکس پروفایل"}
                </span>
              </div>
