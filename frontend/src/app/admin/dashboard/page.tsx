...
import {
  fetchAdminStats, fetchMyAdminEvents, fetchAdminAnalytics,
  AdminEventStat, ApiEvent, isAdminPhone
} from "@/lib/api";
import {
  BarChart2, Plus, Calendar, Users, TrendingUp,
  ChevronRight, Eye, Edit, DollarSign, Activity,
  PieChart, ArrowUpRight, Trash2
} from "lucide-react";
import { useRouter } from "next/navigation";

// ── SVG Bar Chart ──────────────────────────────────────────────────────────
function BarChartSVG({ data, color = "#FF6B00" }: { data: { label: string; value: number }[]; color?: string }) {
  if (!data.length) return <p className="text-slate-500 text-sm text-center py-6">داده‌ای موجود نیست</p>;
  const max = Math.max(...data.map((d) => d.value), 1);
  const H = 120; const W = 100;

  return (
    <div className="overflow-x-auto">
      <svg width={Math.max(data.length * 52, 300)} height={H + 40} style={{ minWidth: "100%" }}>
        {data.map((d, i) => {
          const barH = Math.round((d.value / max) * H);
          const x = i * 52 + 10;
          return (
            <g key={i}>
              <rect x={x} y={H - barH} width={36} height={barH}
                fill={color} rx={4} opacity={0.85} />
              <text x={x + 18} y={H - barH - 6} textAnchor="middle"
                fill="#94a3b8" fontSize={10}>{d.value}</text>
              <text x={x + 18} y={H + 14} textAnchor="middle"
                fill="#64748b" fontSize={9} className="truncate">{d.label.slice(0, 6)}</text>
            </g>
          );
        })}
      </svg>
  ...
  return (
    <div className="flex items-center gap-4 flex-wrap">
      <svg width={140} height={140}>
        ...
        <text x={cx} y={cy + 5} textAnchor="middle" fill="white" fontSize={11} fontWeight="bold">{total}</text>
      </svg>
      <div className="space-y-1.5 flex-1">
        {slices.map((s, i) => (
          <div key={i} className="flex items-center gap-2 text-xs">
            ...
            <span className="text-white font-bold mr-auto">{Math.round(s.value / total * 100)}٪</span>
          </div>
        ))}
      </div>
    </div>
      ...
      {events.map((ev) => (
        <div key={ev.eventId}>
          <div className="flex justify-between items-center mb-1">
            ...
            <span className="text-xs font-black text-orange-400">{Math.round(ev.successRate)}٪</span>
          </div>
...
const CARD = { background: "linear-gradient(145deg, #1B2A4A, #132038)", border: "1px solid rgba(255,255,255,0.08)", boxShadow: "0 8px 24px rgba(0,0,0,0.3)" };
...
export default function AdminDashboardPage() {
  ...
  const catColors = ["#FF6B00","#3B82F6","#10B981","#8B5CF6","#F59E0B","#EF4444","#06B6D4","#EC4899"];
  const catData = (analytics?.categoryBreakdown || []).map((c: any, i: number) => ({
    label: c.category, value: c.count, color: catColors[i % catColors.length]
  }));
  const bookingMonths = (analytics?.bookingsPerMonth || []).map((m: any) => ({ label: m.month, value: m.count }));
  const revenueSparkline = (analytics?.revenuePerMonth || []).map((m: any) => m.revenue);
  ...
  return (
    <div className="max-w-3xl mx-auto pb-24 space-y-5" dir="rtl">
        ...
        <div className="flex gap-2 mt-4 flex-wrap">
          {[
            ...
            <Link key={href} href={href}
              ...
              style={{ background: "rgba(255,107,0,0.12)", color: "#FF9A3C", border: "1px solid rgba(255,107,0,0.2)" }}>
              <Icon size={13} />{label}
            </Link>
          ))}
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          { label: "کل رویدادها", value: stats?.totalEvents || analytics?.totalEvents || 0, icon: Calendar, color: "#FF6B00" },
          { label: "کل رزروها", value: (stats as any)?.totalBookings || analytics?.totalBookings || 0, icon: Activity, color: "#3B82F6" },
          { label: "کل کاربران", value: (stats as any)?.totalUsers || analytics?.totalUsers || 0, icon: Users, color: "#10B981" },
          { label: "میانگین موفقیت", value: `${Math.round(stats?.avgSuccessRate || 0)}٪`, icon: TrendingUp, color: "#F59E0B" },
        ].map(({ label, value, icon: Icon, color }) => (
          <div key={label} className="rounded-2xl p-4 relative overflow-hidden" style={CARD}>
            ...
            <p className="text-[11px] font-medium" style={{ color: "rgba(255,255,255,0.55)" }}>{label}</p>
          </div>
        ))}
      </div>
      ...
      <Link href="/admin/events/new"
        ...
        style={{ background: 'linear-gradient(135deg, rgba(255,107,0,0.2), rgba(255,107,0,0.08))', border: '1px solid rgba(255,107,0,0.35)', boxShadow: '0 4px 24px rgba(255,107,0,0.15)' }}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-3xl"
              style={{ background: 'rgba(255,107,0,0.2)' }}>🤝</div>
            <div>
              <h3 className="text-lg font-black text-white">+ ایجاد همنشینی جدید</h3>
              <p className="text-sm text-orange-300 mt-0.5">تصویر، قیمت، شهر، دسته‌بندی و زمان را تعیین کنید</p>
              <div className="flex gap-2 mt-2">
                {["آپلود تصویر", "انتخاب شهر", "تعیین قیمت", "دسته‌بندی"].map(f => (
                  <span key={f} className="px-2 py-0.5 rounded-full text-[10px] font-bold text-orange-300" style={{ background: 'rgba(255,107,0,0.15)' }}>{f}</span>
                ))}
              </div>
            </div>
          </div>
          <div className="w-10 h-10 rounded-2xl flex items-center justify-center flex-shrink-0" style={{ background: 'rgba(255,107,0,0.2)' }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="rounded-3xl p-5" style={CARD}>
          ...
          <BarChartSVG data={bookingMonths.length ? bookingMonths : [
            { label: "فروردین", value: 12 }, { label: "اردیبهشت", value: 19 },
            { label: "خرداد", value: 8 }, { label: "تیر", value: 25 },
            { label: "مرداد", value: 31 }, { label: "شهریور", value: 22 },
          ]} />
        </div>
        ...
        <div className="rounded-3xl p-5" style={CARD}>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-black text-white text-sm">توزیع دسته‌بندی‌ها</h3>
          </div>
          <DonutChart data={catData.length ? catData : [
              ...
              <div key={ev.id}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs mt-0.5" style={{ color: "rgba(255,255,255,0.45)" }}>
                    ظرفیت: {ev.capacity} نفر | {ev.current_bookings || ev.reservedCount || 0} رزرو
                ...
                <div className="flex gap-2">
                  ...
                  <Link href={`/admin/attendance/${ev.id}`}
