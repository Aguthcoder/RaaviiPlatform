...
import { fetchAllBookings, updateBookingStatus, isAdminPhone, Booking } from "@/lib/api";
...
export default function AdminBookingsPage() {
  ...
  const load = () => {
    setLoading(true);
    fetchAllBookings({ status: statusFilter === "all" ? undefined : statusFilter })
    ...
    <div className="max-w-2xl mx-auto pb-24 space-y-5 relative z-10" dir="rtl">
      <div className="rounded-3xl p-6" style={CARD}>
        ...
        <div className="flex gap-2 mt-3">
          {[["all","همه"],["pending","در انتظار"],["confirmed","تأیید شده"],["cancelled","لغو شده"]].map(([v, l]) => (
            <button key={v} onClick={() => setStatusFilter(v)}
              ...
              style={statusFilter === v ? {} : { background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)" }}>
              {l}
            </button>
          ))}
        </div>
          ...
          {bookings.map((b) => (
            <div key={b.id} className="rounded-2xl p-4" style={CARD}>
              <div className="flex items-center justify-between gap-2 mb-2">
                <div>
                  <p className="font-black text-white text-sm">{(b as any).eventTitle || b.eventId || "رزرو"}</p>
                  <p className="text-xs mt-0.5" style={{ color: "rgba(255,255,255,0.45)" }}>
                    {(b as any).userName || "کاربر"} · {b.createdAt ? new Date(b.createdAt).toLocaleDateString("fa-IR") : ""}
                  </p>
                </div>
                <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full ${
                  b.status === "confirmed" ? "bg-green-500/15 text-green-400 border border-green-500/25" :
                  b.status === "cancelled" ? "bg-red-500/15 text-red-400 border border-red-500/25" :
                  "bg-yellow-500/15 text-yellow-400 border border-yellow-500/25"
                }`}>
                  {b.status === "confirmed" ? "تأیید" : b.status === "cancelled" ? "لغو" : "در انتظار"}
                </span>
              </div>
