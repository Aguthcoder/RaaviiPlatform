...
export class CafeAccessService {
  ...
  async login(username: string, password: string) {
    const cafe = await this.cafeRepo.findOne({ where: { username, is_active: true } });
  ...
  async getAttendanceList(eventId: string, cafeId: string) {
    ...
    if (event.city !== cafe.city) throw new UnauthorizedException('این رویداد در شهر شما نیست');
    const bookings = await this.bookingRepo.find({
      where: { event_id: eventId, payment_status: 'paid' },
      relations: ['user'],
    });
    return {
      event: { id: event.id, title: event.title, start_date: event.start_date, location: event.location },
  ...
  async markAttendance(eventId: string, cafeId: string, attendances: { userId: string; attended: boolean }[]) {
    const cafe = await this.cafeRepo.findOne({ where: { id: cafeId } });
    if (!cafe) throw new NotFoundException('کافه یافت نشد');
    const event = await this.eventRepo.findOne({ where: { id: eventId } });
    if (!event) throw new NotFoundException('رویداد یافت نشد');
    if (event.city !== cafe.city) throw new UnauthorizedException('دسترسی غیرمجاز');
    let count = 0;
    for (const a of attendances) {
      const booking = await this.bookingRepo.findOne({ where: { event_id: eventId, user_id: a.userId } });
      if (!booking) continue;
  ...
  async computeVenuePriceTier(eventId: string): Promise<'budget' | 'medium' | 'expensive'> {
