...
export class WebhookController {
  ...
  async n8nWebhook(
    ...
    this.verify(secret || '');
    console.log('[N8N WEBHOOK] Received:', payload?.type, JSON.stringify(payload).slice(0, 200));
  ...
  async storeVector(
    ...
    this.verify(secret || '');
    console.log('[N8N VECTOR] Received vector payload for userId:', payload?.userId);
